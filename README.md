...
- [x] 拡張: 忘却曲線（時間経過で理解が減衰）＋内部状態をキャラの気分・表情として可視化
- [x] 拡張: 忘却を会話にも反映（忘れた概念を「覚えなおし」として自分から聞き直す／気分が口調に出る）
- [x] 拡張: teach 中にキャラの状態を常時描画するライブパネル（表情/習熟度/頭の中が毎ターン更新）
- [x] 拡張: 人格の永続化（persona.json）で口調・性格のブレを防止
